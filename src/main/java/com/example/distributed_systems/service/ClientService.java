...
public class ClientService {
    ...
    public ClientPutResponse put(String key, String value) {
       RouteResponse mapping = restTemplate.getForObject(CONTROLLER_URL + "?key=" + key, RouteResponse.class);
        ...
        for (int i = 0; i < Math.min(2, replicas.size()); i++) {
            ...
            String url = "http://" + worker.getHost() + ":" + worker.getPort() + "/worker/put";
            try {
        ...
        if (replicas.size() > 2) {
            ...
            CompletableFuture.runAsync(() -> {
                try {
                    String url = "http://" + asyncWorker.getHost() + ":" + asyncWorker.getPort() + "/worker/put";
                    PutRequest req = new PutRequest();
                    req.setKey(key);
                    req.setValue(value);
    ...
    public ClientGetResponse get(String key) {
       RouteResponse mapping = restTemplate.getForObject(CONTROLLER_URL + "?key=" + key, RouteResponse.class);
        ...
        String url = "http://" + primary.getHost() + ":" + primary.getPort() + "/worker/get?key=" + key;
