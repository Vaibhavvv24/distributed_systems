...
export default function WorkerFrontend2() {
  ...
  async function fetchStatus() {
    ...
    setStatus(null);
    try {
      const res = await fetch(`http://localhost:8082/v1/worker/status`);
  ...
  async function handleGet(e) {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8082/v1/worker/get?${params.toString()}`);
      if (!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
  ...
  async function handlePut(e) {
    ...
    try {
      const res = await fetch(`http://localhost:8082/v1/worker/put`, {
        method: "POST",
  ...
  //   try {
  //     const res = await fetch(`http://localhost:8082/v1/worker/replicate/${encodeURIComponent(repKey.trim())}`, {
  //       method: "PUT",
  //       headers: { "Content-Type": "application/json" },
  ...
  //   try {
  //     const res = await fetch(`http://localhost:8082/v1/worker/health`);
  //     if (!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white py-10">
      <div className="max-w-4xl mx-auto px-4">
        <header className="mb-8">
          <h1 className="text-2xl font-extrabold text-slate-800">Worker 2 Dashboard</h1>
